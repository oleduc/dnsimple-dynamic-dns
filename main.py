...
class DNSUpdater:
    ...
    def collect_pages(self, api_call: Callable[[int], Any]) -> List[Any]:
    ...
    def get_current_external_ip(self, ip_service_url: str) -> str:
    ...
    def get_current_config_zone(self, zone_name: str) -> Dict[str, Any]:
        """Finds the configuration for the specified DNS zone."""
        filtered_config_zones = [config_zone for config_zone in self.config['zones'] if config_zone['name'] == zone_name]
    ...
    def update_dns_records(self):
        ...
        public_ip = self.get_current_external_ip(self.config['ip_service_url'])

        if self.current_public_ip == public_ip:
            logging.info("Public IP has not changed. Skipping DNS update.")
            return
        else:
            self.current_public_ip = public_ip

        dnsimple_client = Client(access_token=self.config['api_token'])
        zones = self.collect_pages(lambda page: dnsimple_client.zones.list_zones(self.config['account_id'], page=page))

        for zone in zones:
            try:
                configured_zone = self.get_current_config_zone(zone.name)
            except Exception as e:
                logging.warning(e)
                continue

            records = self.collect_pages(
                lambda page: dnsimple_client.zones.list_records(self.config['account_id'], zone.name, page=page))
...
def main():
    ...
    updater = DNSUpdater(config)

    try:
        while True:
            updater.update_dns_records()
            time.sleep(config['update_frequency_seconds'])
